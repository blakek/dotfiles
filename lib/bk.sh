...
# Terminal color helpers shared across scripts.
# Sourced (not exported) so they stay local to the current shell.
...
bk.hex_to_rgb() {
	...
	if [[ $hex =~ ^#([0-9a-fA-F]{2})([0-9a-fA-F]{2})([0-9a-fA-F]{2})$ ]]; then
		...
		# Double single digit hex values (e.g., #abc -> #aabbcc)
...
# ANSI escape sequences used by bk.term.*
